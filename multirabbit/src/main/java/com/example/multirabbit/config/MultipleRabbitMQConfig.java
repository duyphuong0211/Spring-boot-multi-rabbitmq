...
public class MultipleRabbitMQConfig {
    ...
    @Bean(name = "v2ConnectionFactory")
    public ConnectionFactory hospSyncConnectionFactory(@Value("${v2.spring.rabbitmq.host}") String host,
                                                       @Value("${v2.spring.rabbitmq.port}") int port,
                                                       @Value("${v2.spring.rabbitmq.username}") String username,
                                                       @Value("${v2.spring.rabbitmq.password}") String password,
                                                       @Value("${v2.spring.rabbitmq.virtual-host}") String virtualHost) {
    ...
    public RabbitTemplate firstRabbitTemplate(
        ...
        v2RabbitTemplate.setConfirmCallback((correlationData, ack, s) -> {
            if (!ack) {
// LOGGER.info("{} Send RabbitMQ message ack confirmation failed: [{}]", this.name, JSON.toJSONString(object));
            } else {
...
// LOGGER.error("{} Sending RabbitMQ message returnedMessage, an exception occurred, Exchange does not exist or sent to Exchange but not sent to the Queue, message: [{}], code[{}], s[{}], exchange[{}], routingKey[{}]", new Object[]{this.name, JSON.toJSONString(message), JSON.toJSONString(code), JSON.toJSONString(s), JSON.toJSONString(exchange), JSON.toJSONString(routingKey)});
        });
